...
parseNSymbol = do 
       ...
       symb <- symbol
       _ <- emptySpace
       num <- many1  (numeral <* Pc.try spaces)
       _ <- aspC
...
parseAttribute = do
    ...
    case attr of
      ...
      Just val -> return $ AttributeVal key valq-}
...
parseDeclareSort = do
  ...
  _ <- emptySpace
  _ <- aspC
  return $ DeclareSort symb (read num :: Int)
